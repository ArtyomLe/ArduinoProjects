...
void setup() {
  size(400, 400);                 // Размеры основного рабочего окна
  cp5 = new ControlP5(this);      // Базовая инициализация

  // Кнопки интерфейса
  cp5.addButton("open").linebreak();               // Открываем порт к которому подключено ардуино
  cp5.addButton("closePort").linebreak();          // Закрываем порт к которому подключено ардуино
  cp5.addToggle("led").setMode(ControlP5.SWITCH);  // Переключатель встроенного в плату LED
  cp5.addToggle("fan").setMode(ControlP5.SWITCH);  // Переключатель вентилятора
  cp5.addToggle("bulb").setMode(ControlP5.SWITCH).linebreak(); // Переключатель лампы
  cp5.addScrollableList("comlist").close();        // Закрываем выпадающее меню чтобы не загараживало остальные кнопки основного меню
  cp5.addKnob("knob")                              // Добавляем крутилку для серво
    .setRange(0, 180)
    .setRadius(30)
    ;
  cp5.addColorWheel("picker", 10, 270, 100);   // Цветовое колесо для RGB диода
  ...
  cp5.addButton("send").setPosition(265, 150);

  m = new Meter(this, 150, 10);                // Создаём шкалу библиотеки Meter
  m.setMeterWidth(200);                        // Задаём ширину
  m.setUp(0, 1023, 0, 100, -180, 0);
  String[] scaleLabels = {"0", "20", "40", "60", "80", "100"};
  m.setScaleLabels(scaleLabels);
...
void led(int val) {
  serial.write("0," + val + ";");      // Переключатель LED
}
void fan(int val) {                 
  serial.write("3," + val + ";");      // Переключатель вентилятора
}
void bulb(int val) {
  serial.write("4," + val + ";");      // Переключатель реле
}
void send() {
  serial.write("5," + cp5.get(Textfield.class, "input").getText() + ";");      // Отправляем текст
}
void picker(int col) {        // Складываем строки по след. протоколу "1,0-255,0-255,0-255"
  String str = "1,";          // Обращаемся к RGB диоду
  str += int(red(col));       // Добавляем цвета через одноимённые функции(red, green, blue) и приравниваем к int (изначально float);
