...
int random2;
int a;              // Straight obstacle
int a1;             // Left obstacle
int a2;             // Right obstacle
...
void Car_front()
{
  digitalWrite(MR_Ctrl,LOW);  // Clockwise rotation 
  analogWrite(MR_PWM,200);    // Speed 200 (range 0-255)
  digitalWrite(ML_Ctrl,LOW);
  analogWrite(ML_PWM,200);
}
void Car_back()
{
  digitalWrite(MR_Ctrl,HIGH); // Anticlockwise rotation 
  analogWrite(MR_PWM,200);    // Speed 200 (range 0-255)
...
void Car_left()
{
  ...
  digitalWrite(ML_Ctrl,HIGH); // Anticlockwise rotation
...
void Car_right()
{
  digitalWrite(MR_Ctrl,HIGH); // Anticlockwise rotation
  analogWrite(MR_PWM,255);
  digitalWrite(ML_Ctrl,LOW);  // Clockwise rotation
  analogWrite(ML_PWM,255);
}
void Car_Stop()
{
...
void setup(){
  ...
  pinMode(Trig, OUTPUT);      // Передача
  pinMode(Echo, INPUT);       // Приём
...
void loop(){
  ...
  if (a < 20)                 //when the front distance detected is less than 20 
  {
      ...
      {   
        a1 = checkdistance();               //assign the left distance detected by ultrasonic sensor to variable a1(читаем 10 раз)
      }
      ...
      {
        a2 = checkdistance();               //assign the right distance detected by ultrasonic sensor to variable a2(читаем 10 раз)
      }
